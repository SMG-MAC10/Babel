...
public class BattleChar : MonoBehaviour
{
    ...
    public List<CounterSkills> phyDamagedStack = new List<CounterSkills>(); // 공격 받을시 사용되는 스킬들
    public List<CounterSkills> mgcDamagedStack = new List<CounterSkills>(); // 공격 받을시 사용되는 스킬들
    ...
    public float SetMagicDamage(BattleChar user, float damCoef, int lv, float coef)
    {
        ...
        totalDamage = ((user.GetMgcATK() * damCoef) - (user.GetMgcATK() * damCoef * (GetMgcDEF() / (50 + (100 - BM.floorLevel) * 1.5f + GetMgcDEF())) * 1.4f)) * coef;
    ...
    public float SetPhysicalDamage(BattleChar user, float damCoef, int lv, float coef)
    {
        float totalDamage = .0f;
        if (DC.curHP <= 0) return 0;
        totalDamage = ((user.GetPhyATK() * damCoef) - (user.GetPhyATK() * damCoef * (GetPhyDEF() / (50 + (100 - BM.floorLevel) * 1.5f + GetPhyDEF())) * 1.4f)) * coef;
        print("totalDamage = ("+user.GetPhyATK() + " * " + damCoef + " * (1 + (" + lv + " * 0.005f))) - (" + user.GetPhyATK() + " * " + damCoef + " * (" + GetPhyDEF() + " / (50 + (100 - " + BM.floorLevel + ") * 1.5f + " + GetPhyDEF() + ")) * 1.4f) *" + coef);
    ...
    void Death() {
        ...
        if (tg == TARGETTAG.FRIEND) tg = TARGETTAG.FRIENDDEAD;
        else tg = TARGETTAG.ENEMYDEAD;
