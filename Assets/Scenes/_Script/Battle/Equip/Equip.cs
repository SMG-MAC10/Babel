...
public class Equip {
    ...
    public Equip(Equip e) {
        level = e.level;
        img = e.img; // 장비 이미지 URL
        name = e.img; // 장비이름

        type = e.type; // 장비종류 : 투구 - 갑옷 - 신발 - 무기 - 보조무기
        cls = e.cls; // 직업제한 : 도적 - 궁수 - 법사 - 기사 - 사제
        grade = e.grade; // 장비등급 : 노말 - 레어 - 유니크 - 미스틱

        maxHP = e.maxHP; // 체력
        maxMP = e.maxMP; // 마나
        incMP = e.incMP; // 마나회복
        speed = e.speed; //incAP; // 속도회복
        phyATK = e.phyATK; // 물리공격력
        phyDEF = e.phyDEF; // 물리방어력
        macATK = e.macATK; // 마법공격력
        macDEF = e.macDEF; // 마법방어력
        phyATKRate = e.phyATKRate; // 물리공격력
        phyDEFRate = e.phyDEFRate; // 물리방어력
        macATKRate = e.macATKRate; // 마법공격력
        macDEFRate = e.macDEFRate; // 마법방어력
        healEFC = e.healEFC; // 회복효율
        critical = e.critical; // 치명타확률
      }
...
public class EquipSet {
    ...
    public EquipSet(EquipSet e) {
        set = new Equip[5];

        for (int i = 0; i < 5; i++) {
            set[i] = new Equip(e.set[i]);
        }
    }
