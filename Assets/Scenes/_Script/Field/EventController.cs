...
public class EventController : MonoBehaviour
{
    ...
    public GameObject jailEventController;
    public GameObject requestNextFloorPanel;
    public GameObject controlBlocker;
    public GameObject battleForceTerminatePanel;
    ...
    void Start()
    {
        ...
        gameManager.PlayBGM(SoundFilePath.FieldBGM);
        ...
        if (PlayerPrefsX.GetBool(PrefsEntity.ForceTerminateInBattle, false))
        {
            Utility.ObjectVisibility(battleForceTerminatePanel, true);
        }
    ...
    {
        //Debug.Log("Exit");
        GameObject boss = GameObject.FindGameObjectWithTag("FieldBoss");
        if (boss != null & PlayerPrefsX.GetBool(PrefsEntity.BossClearFlag, false)) // Boss battle clear
        {
            PlayerPrefsX.SetBool(PrefsEntity.BossClearFlag, false);
            DestroyImmediate(boss);
        }
        if (boss != null & PlayerPrefs.GetInt(PrefsEntity.CurrentFloor) % 20 == 1) // Boss encounter
        {
            PlayerPrefsX.SetBool(PrefsEntity.ForceTerminateInBattle, true);
            PlayerPrefsX.SetBool(PrefsEntity.BattleToBoss, true);
            LoadingManager.LoadSceneFieldToBattle();
        }
        if (boss != null & PlayerPrefs.GetInt(PrefsEntity.CurrentFloor) % 20 == 11) // Semi boss encounter
        {
            PlayerPrefsX.SetBool(PrefsEntity.ForceTerminateInBattle, true);
            PlayerPrefsX.SetBool(PrefsEntity.BattleToBoss, true);
            LoadingManager.LoadSceneFieldToBattle();
        }
        gameManager.PlaySound(SoundFilePath.ArriveExit);
    ...
    {
        Debug.Log("Trap");
        gameManager.PlaySound(SoundFilePath.OccurTrap);
        PlayerPrefsX.SetBool(PrefsEntity.ForceTerminateInTrap, true);
        Vector2 eventCoordination = new Vector2(currentX, currentY);
        PlayerPrefsX.SetVector2(PlayerPrefs.GetString(eventCoordination.ToString()), new Vector2(-1, -1));
        DestroyImmediate(eventObject.gameObject);
        switch (currentLevel)
    ...
    {
        gameManager.PlaySound(SoundFilePath.OccurJail);
        Debug.Log("Jail");
        PlayerPrefsX.SetBool(PrefsEntity.ForceTerminateInJail, true);
        
        jailEventController.GetComponent<JailEventController>().OccurJailEvent();
    }

    public void JailEventEnd()
    {
        Vector2 eventCoordination = new Vector2(currentX, currentY);
        PlayerPrefsX.SetVector2(PlayerPrefs.GetString(eventCoordination.ToString()), new Vector2(-1, -1));
        //Utility.ObjectVisibility(eventObject.gameObject, false);
        DestroyImmediate(fieldEventContainer[currentX, currentY].gameObject);
        switch (currentLevel)
        {
            case 0:
                EventNormal();
        ...
        fieldEventFlag[currentX, currentY] = FieldEventType.Nothing;
    }
    ...
    private void OccurBox(ref FieldEventType eventFlag, ref Transform eventObject)
    {
        Debug.Log("Box");
        Vector2 eventCoordination = new Vector2(currentX, currentY);
        PlayerPrefsX.SetBool(PrefsEntity.ForceTerminateInEvent, true);
        PlayerPrefsX.SetVector2(PlayerPrefs.GetString(eventCoordination.ToString()), new Vector2(-1, -1));
        DestroyImmediate(eventObject.gameObject);
        Utility.ObjectVisibility(boxPanel, true);
        eventFlag = FieldEventType.Nothing;
