...
public class DecomposePanelController : MonoBehaviour
{
    ...
    public GameObject decomposeResIcon;
    ...
    private void DecomposeListUpdate()
    {
        ...
        decomposeScroll.text = "보유한 장비 분해 주문서 : " + PlayerPrefs.GetInt(PrefsEntity.DecomposeScroll);
        equipName.text = "";
        equipGrade.text = "";
        decomposeReq.text = "";
        decomposeRes.text = "";
        decomposeResIcon.GetComponent<Image>().color
            = new Color(0, 0, 0, 0);
    ...
    public void OnClickItem(int index)
    {
        ...
        decomposeResIcon.GetComponent<Image>().sprite
            = Resources.Load<Sprite>(Resource.ScrollEquip);
        decomposeResIcon.GetComponent<Image>().color
            = new Color(1, 1, 1, 1);
    ...
    {
        gameManager.PlaySound(SoundFilePath.ButtonTab);
        int poseScroll = PlayerPrefs.GetInt(PrefsEntity.DecomposeScroll);
        if(selectedEquipIndex == -1 | poseScroll < reqScroll)
        {
            return;
        }
        ...
        PlayerPrefs.SetInt(PrefsEntity.DecomposeScroll, poseScroll - reqScroll);
        PlayerPrefs.SetInt(PrefsEntity.EquipmentUpgradeScroll, 
            PlayerPrefs.GetInt(PrefsEntity.EquipmentUpgradeScroll) + resScroll);
        DecomposeListUpdate();
    }
}
