...
public class MapGenerator : MonoBehaviour
{
    ...
    public Transform fieldBossPrefab;
    ...
    public void GenerateExit()
    {
        for (int i = 0; i < 1; i++)
        {
            ...
            if (fieldEventFlag[(int)exit.x, (int)exit.y] == 0)
            {
                ...
                //newTile.localScale = Vector3.one * 0.3f;
                if(PlayerPrefs.GetInt(PrefsEntity.CurrentFloor) % 20 == 1
                    | PlayerPrefs.GetInt(PrefsEntity.CurrentFloor) % 20 == 11)
                {
                    Transform boss = Instantiate(fieldBossPrefab, eventPosition, Quaternion.Euler(Vector3.right * 0), newTile) as Transform;
                    boss.localRotation = Quaternion.Euler(new Vector3(10, -40, 0));
                    boss.localScale = new Vector3(0.1f, 0.1f, 0.1f);
                }
